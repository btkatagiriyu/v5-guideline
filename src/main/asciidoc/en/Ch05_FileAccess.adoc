...
An example for implementatiion of ``FieldExtractor``, refer to <<Ch05_FileAccess_FixedLength_Output,Output of Fixed-length file>> where a sample is described using full-width character.
|icon:check[]
|Nothing
...
To use ``ItemReader`` bean defined in implementation class of ``Tasklet``, refer to <<Ch03_CreateTaskletJob.adoc#Ch03_CreateTaskletJob_HowToUse_Impl,Creating a tasklet-oriented job>>
